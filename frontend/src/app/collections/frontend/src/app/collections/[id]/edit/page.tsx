...
import {
  ArrowLeft, Plus, X, Eye, Save, Loader2, Sparkles, Globe,
} from "lucide-react";
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([col, genreList]) => {
        ...
        setRules(col.rules.map((r) => ({ field: r.field, value: r.value })));
        setGenres(Array.isArray(genreList) ? genreList : (genreList as any).results ?? []);
      })
      .catch(console.error)
  ...
  return (
    <div className="pt-24 pb-20 px-6 md:px-10 lg:px-20 max-w-[1440px] mx-auto">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold font-display">Edit Collection</h1>
          <p className="text-sm text-white/30">Update rules and details</p>
        </div>
      </div>
          ...
          {previewLoading ? (
            ...
            <div className="text-center py-20 border border-dashed border-white/[0.06] rounded-xl">
              ...
              <p className="text-sm text-white/25">
                Add rules to see matching movies
              </p>
            </div>
          )}
        </div>
